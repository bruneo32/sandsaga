...
typename Earcut<N>::Node *Earcut<N>::linkedList(const Ring &points,
	...
	// link points into circular doubly-linked list in the specified winding
	// order
	if (clockwise == (sum > 0)) {
		for (i = 0; i < len; i++)
...
typename Earcut<N>::Node *Earcut<N>::findHoleBridge(Node *hole,
	...
	// find a segment intersected by a ray from the hole's leftmost Vertex to
	...
	do {
		if (hy <= p->y && hy >= p->next->y && p->next->y != p->y) {
			...
			if (x <= hx && x > qx) {
				...
				if (x == hx)
					return m; // hole touches outer segment; pick leftmost
							  // endpoint
			}
		}
		p = p->next;
	...
	// otherwise choose the Vertex of the minimum angle with the ray as
	// connection Vertex
	...
	do {
		if (hx >= p->x && p->x >= mx && hx != p->x &&
			pointInTriangle(hy < my ? hx : qx, hy, mx, my, hy < my ? qx : hx,
							hy, p->x, p->y)) {
